...
* **HTTP/1.1 301 Moved Permanently** - код состояния из группы перенаправлений, сообщающий о том, что сайт перемещен на постоянной основе (адрес нового расположения указывается в поле Location).
* **cache-control: no-cache, no-store, must-revalidate** - пареметры кэширования. Здесь:
  ...
  - `must-revalidate` - говорит о том, что каждый запрос нужно делать заново, и ни при каких условиях не предоставлять пользователю закешированный контент. Имеет преимущество над всеми другими директивами, которые разрешают кэширование. При использовании с `max-age`, директивы устанавливают время, в течение которого данные в кэше будут считаться актуальными;
...
```commandline
$ dig ns +short dns.google > dig.file && dig a +short -f dig.file > ip.file 
$ cat ip.file 
216.239.36.114
216.239.32.114
216.239.38.114
216.239.34.114
$ cat ip.file | while read addr; do dig -x $addr +short; done
ns3.zdns.google.
ns1.zdns.google.
ns4.zdns.google.
ns2.zdns.google.
